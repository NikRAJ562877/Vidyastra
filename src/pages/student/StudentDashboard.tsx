...
const navItems = [
  { label: 'Dashboard', href: '/student', icon: <ClipboardList className="h-4 w-4" /> },
  ...
  { label: 'Rankings', href: '/student/rankings', icon: <Trophy className="h-4 w-4" /> },
...
const StudentDashboard = () => {
  ...
  const attendancePercent = studentAttendance.length > 0 ? (presentCount / studentAttendance.length) * 100 : 0;
  ...
  return (
    <DashboardLayout title="Student Dashboard" navItems={navItems} userName={user.name || 'Student'} userRole="student">
      {/* Rank Banner */}
      <div className="gradient-primary rounded-xl p-5 mb-6 text-primary-foreground">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm text-primary-foreground/70">Your Class Rank</p>
            <p className="text-4xl font-heading font-bold">#{rank}</p>
            <p className="text-sm text-primary-foreground/80 mt-1">out of {classmates.length} students in {student.class}</p>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <StatCard title="Average Score" value={`${avgMarks.toFixed(1)}%`} icon={<BookOpen className="h-5 w-5" />} />
        <StatCard title="Attendance" value={`${attendancePercent.toFixed(0)}%`} icon={<CalendarCheck className="h-5 w-5" />} />
        <StatCard title="Subjects" value={studentMarks.length} icon={<ClipboardList className="h-5 w-5" />} />
        <StatCard title="Total Marks" value={totalMarks} icon={<Trophy className="h-5 w-5" />} />
      </div>
          ...
          <div className="bg-card rounded-xl border border-border shadow-card p-5">
            ...
            <div className="flex items-end gap-4">
              <div className="flex-1">
                ...
                <p className="text-sm text-muted-foreground mt-2">{presentCount} of {studentAttendance.length} days present</p>
              </div>
              <p className="text-2xl font-heading font-bold">{attendancePercent.toFixed(0)}%</p>
            </div>
          </div>
            ...
            <div className="space-y-3">
              {studentPayments.map(p => (
                <div key={p.id} className="flex items-center justify-between p-3 rounded-lg bg-muted">
                  <div>
                    <p className="text-sm font-medium">Installment {p.installment}/{p.totalInstallments}</p>
                    <p className="text-xs text-muted-foreground">Due: {p.dueDate}</p>
                  </div>
                  <div className="text-right">
                    <p className="font-heading font-bold">₹{p.amount.toLocaleString()}</p>
                    <Badge variant={p.status === 'paid' ? 'default' : 'destructive'} className="text-xs">
                      {p.status === 'paid' ? 'Paid' : p.status === 'pending' ? 'Pending' : 'Not Paid'}
                    </Badge>
                  </div>
                </div>
              ))}
