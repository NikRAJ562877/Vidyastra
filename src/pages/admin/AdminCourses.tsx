...
import { Trash2, Edit2, Plus, ClipboardList, UserPlus, Users, GraduationCap, Trophy, CalendarCheck, IndianRupee, BookOpen } from 'lucide-react';
...
const AdminCourses = () => {
    ...
    return (
        <DashboardLayout title="Manage Courses" navItems={navItems} userName="Admin" userRole="admin">
            <div className="flex justify-between items-center mb-6">
                <div>
                    <h2 className="text-xl font-heading font-bold">All Courses</h2>
                    <p className="text-sm text-muted-foreground">Create and manage courses for enrollment</p>
                </div>
                        ...
                        {courses.length > 0 ? (
                            courses.map(course => (
                                <TableRow key={course.id}>
                                    ...
                                    <TableCell><Badge variant="secondary">{course.batch}</Badge></TableCell>
                                    ...
                                    <TableCell className="text-right">
                                        <div className="flex justify-end gap-2">
                                            ...
                                            <Button variant="ghost" size="sm" onClick={() => handleDelete(course.id)} className="text-destructive hover:text-destructive hover:bg-destructive/10">
                ...
                <DialogContent className="sm:max-w-[500px]">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4 py-4">
                        <div className="grid gap-2">
                            ...
                            <Input id="name" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} placeholder="e.g. JEE Advanced Physics" required />
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="grid gap-2">
                                ...
                                <Input id="fee" type="number" value={formData.fee} onChange={e => setFormData({ ...formData, fee: Number(e.target.value) })} required />
                            </div>
                            <div className="grid gap-2">
                                <Label htmlFor="duration">Duration *</Label>
                                <Input id="duration" value={formData.duration} onChange={e => setFormData({ ...formData, duration: e.target.value })} placeholder="e.g. 6 Months" required />
                            </div>
                        </div>
                        <div className="grid gap-2">
