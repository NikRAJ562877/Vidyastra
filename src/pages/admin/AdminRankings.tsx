...
const AdminRankings = () => {
  ...
  }, [selectedTestId, selectedClass, students, marks]);

  return (
    <DashboardLayout
      title="Marks & Rankings Dashboard"
    ...
    >
      ...
      {selectedTestId && selectedClass ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {rankings.slice(0, 3).map((r, i) => (
              <div
                key={r.id}
                className={cn(
                  "p-6 rounded-2xl border relative overflow-hidden bg-white shadow-sm transition-all hover:shadow-md",
                  i === 0
                    ? "border-amber-200"
                    : i === 1
                      ? "border-slate-200"
              ...
              >
                <div className="flex justify-between items-start relative z-10">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {r.percentage.toFixed(1)}% ({r.totalMarks}/{r.maxMarks})
                    </p>
                  </div>
                  <div
                    className={cn(
                ...
                <div
                  className={cn(
                    "absolute -right-4 -bottom-4 opacity-5",
                    i === 0
                      ? "text-amber-500"
                      : i === 1
                        ? "text-slate-500"
              ...
              <TableBody>
                {rankings.length === 0 ? (
                  ...
                  rankings.map((r) => (
                    <TableRow
                      key={r.id}
                      className={cn(r.rank <= 3 && "bg-primary/5")}
                    >
                      <TableCell className="font-bold">
                        <div
                          className={cn(
                            "w-7 h-7 rounded-sm flex items-center justify-center text-xs",
                            r.rank === 1
        ...
        <div className="bg-white rounded-3xl border border-dashed p-24 text-center">
          ...
          <h2 className="text-2xl font-heading font-bold text-slate-400">
            Class Performance Analytics
          </h2>
          <p className="text-muted-foreground max-w-sm mx-auto mt-2">
            Select an examination and a class to generate comparative rankings
