...
import { BookOpen, Users, CalendarCheck, Trophy, Upload, ClipboardList } from 'lucide-react';
import { Badge } from '@/components/ui/badge';

const navItems = [
  { label: 'Dashboard', href: '/teacher', icon: <ClipboardList className="h-4 w-4" /> },
  { label: 'Upload Notes', href: '/teacher/notes', icon: <Upload className="h-4 w-4" /> },
  { label: 'Upload Marks', href: '/teacher/marks', icon: <BookOpen className="h-4 w-4" /> },
...
const TeacherDashboard = () => {
  ...
  const assignedStudents = students.filter(s => teacher.assignedClasses.includes(s.class));

  return (
    <DashboardLayout title="Teacher Dashboard" navItems={navItems} userName={user.name || 'Teacher'} userRole="teacher">
            ...
            {assignedStudents.map(s => (
              <div key={s.id} className="flex items-center justify-between p-2 rounded-lg hover:bg-muted transition-colors">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {s.category === 'slow_learner' && <Badge variant="destructive" className="text-xs">Slow Learner</Badge>}
                </div>
              </div>
            ))}
          </div>
