    ...
    <div className="flex overflow-hidden py-3 select-none">
      ...
      >
        {duplicatedItems.map((item, idx) => (
          <div
            key={idx}
            className="flex items-center gap-3 bg-card border border-border px-6 py-3 rounded-full shadow-sm hover:shadow-md transition-shadow"
          >
  ...
  return (
    <section className="py-20 bg-muted/30 overflow-hidden">
      ...
      <div className="space-y-4">
        <MarqueeRow items={items1} direction="left" speed={30} />
        <MarqueeRow items={items2} direction="right" speed={35} />
        <MarqueeRow items={items3} direction="left" speed={32} />
      </div>
    </section>
  );
};
